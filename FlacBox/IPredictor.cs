...
namespace FlacBox
{
    ...
    static class PredictorFactory
    {
        ...
        class LpcSecondOrderPredictor : IPredictor
        {
            ...
            public int Next(int x)
            {
                var result = x * coefficient1 + lastX * coefficient2;
                lastX = x;
                return result;
            }
            ...
            public long NextLong(int x)
            {
                var result = x * (long)coefficient1 + lastX * (long)coefficient2;
                lastX = x;
                return result;
            }
        }
        ...
        class LpcPredictor : IPredictor
        {
            ...
            {
                Debug.Assert(coefficients.Length == samples.Length + 1);

                this.coefficients = coefficients;
                order = coefficients.Length;

                this.samples = new int[order - 1];
