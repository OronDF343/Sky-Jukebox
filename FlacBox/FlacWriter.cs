    ...
    public class FlacWriter : IDisposable
    {
        ...
        public FlacWriter(Stream baseStream, bool leaveOpen)
        {
            ...
            if (baseStream is IPageble)
                pageble = (IPageble)baseStream;
            else
                pageble = NullForIPageble.Instance;
            this.leaveOpen = leaveOpen;
            sink = NullFrameSink.Instance;
        }
        ...
        {
            Debug.Assert(metaWriter != null);

            var type = metaWriter.Type;
            if(last) type |= 0x80;
            BaseStream.WriteByte(type);

            var length = metaWriter.Length;
        ...
        {
            var tail = buffer.Length - bufferSize;
        ...
        {
            var channelsCount = Streaminfo.ChannelsCount;
            var samplesPerChannel = count / channelsCount;

            var nonInterleavedSamples = new int[channelsCount][];
        ...
        public void WriteFrame(int[][] samples, SoundChannelAssignmentType channelAssigment)
        {
            ...
            var samplesPerChannel = samples[0].Length;
            ...
            else
            {
                ...
                var samplesAndMethods = estimator.FindBestMethods(
        ...
        public void WriteFrame(FlacMethodAndDataPair[] methods, SoundChannelAssignmentType channelAssignment)
        {
            ...
            var samplesCount = methods[0].Data.Length;
            ...
            var interChannelSamplesTypeIndex = Array.IndexOf(FlacCommons.StaticBlockSizeSamples, samplesCount);
            ...
            var sampleRateTypeIndex = Array.IndexOf(FlacCommons.StaticSampleRates, Streaminfo.SampleRate);
            var sampleRateType = sampleRateTypeIndex > 0 
            ...
            var channelAssignmetType = (int)channelAssignment;

            var sampleSizeInBitsTypeIndex = Array.IndexOf(FlacCommons.StaticSampleSizeInBits, Streaminfo.BitsPerSample);
            var sampleSizeInBitsType = sampleSizeInBitsTypeIndex > 0
            ...
            var frameHeaderData = frameHeader.ToArray();

            var crc8 = CrcUtils.Crc8(0, frameHeaderData);
            ...
            var crc16Seed = CrcUtils.Crc16(CrcUtils.Crc16(0, frameHeaderData), crc8);

            // write channels
            var bitWriter = new FlacBitStreamWriter(BaseStream, crc16Seed);
            ...
            var frameSize = frameHeaderData.Length + 1 + subframesLength + 2;
        ...
        private void WriteSubframe(FlacBitStreamWriter bitWriter, FlacMethodAndDataPair flacMethodAndDataPair)
        {
            ...
            var subframeType = flacMethodAndDataPair.Method.SubframeTypeCode;
        ...
        private static void WriteUtf8Number(Stream stream, int number)
        {
            ...
            {
                var mask = 0x40;
                var parts = new Stack<byte>(4);
                do
                {
                    var part = (number & 0x3F) | 0x80;
                    number >>= 6;
                    parts.Push((byte)part);
                    mask >>= 1;
                } while (number >= mask);
                var firstByte = (0x100 - (mask << 1)) | number;
        ...
        public FlacMethodAndDataPair FindBestMethod(int[] channelSamples, SoundChannelAssignment channelAssigment)
        {
            ...
            var channelBitsPerSample = Streaminfo.BitsPerSample;
        ...
        private class StreaminfoWriter : MetadataBlockWriter
        {
            ...
            public override void Write(Stream s)
            {
                ...
                var ratePlusChannelsAndBits = (uint)(streaminfo.SampleRate << 12 |
