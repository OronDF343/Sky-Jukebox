...
namespace FlacBox
{
    ...
    {        
        ...
        public WaveOverFlacStream(Stream stream, WaveOverFlacStreamMode mode, bool leaveOpen, int compressionLevel)
        {
            ...
            switch (mode)
            {
                case WaveOverFlacStreamMode.Decode:
                    dataSource = ReadFlac(new FlacReader(stream, leaveOpen)); break;
                case WaveOverFlacStreamMode.Encode:
                    dataConsumer = WriteFlac(new FlacWriter(stream, leaveOpen), compressionLevel);
        ...
        public WaveOverFlacStream(FlacReader reader)
        {
            ...
            dataSource = ReadFlac(reader);
            mode = WaveOverFlacStreamMode.Decode;
        }
        ...
        public WaveOverFlacStream(FlacWriter writer, int compressionLevel)
        {
            ...
            dataConsumer = WriteFlac(writer, compressionLevel);
            dataConsumer.MoveNext();
            mode = WaveOverFlacStreamMode.Encode;
        }
        ...
        {
            var read = sampleTransform.PackData(reader.GetValues(), pcmBuffer);           
        ...
        private byte[] CreateWaveHeader()
        {
            ...
            var bytesPerInterChannelSample = (streaminfo.ChannelsCount * streaminfo.BitsPerSample) >> 3;
            var dataLength = streaminfo.TotalSampleCount * bytesPerInterChannelSample;
            totalStreamLength = dataLength + WaveHeaderSize;
            ...
            var bytesPerSecond = streaminfo.SampleRate * bytesPerInterChannelSample;
        ...
        private IEnumerator<WriteRequest> WriteFlac(FlacWriter writer, int compressionLevel)
        {
            try
            {
                ...
                var waveHeader = new byte[RiffHeaderLength];
                var buffer = new ArraySegment<byte>(waveHeader);
                do
                {
                ...
                do
                {
                    var blockHeader = new byte[RiffBlockHeaderLength];
                    ...
                    {
                        var dataBlockSize = BitConverter.ToUInt32(blockHeader, 4);
                        if (streaminfo == null)
                            throw new FlacException("Format block was not found");

                        var bytesPerInterChannelSample = (streaminfo.ChannelsCount * streaminfo.BitsPerSample) >> 3;

                        var totalSamples = dataBlockSize / bytesPerInterChannelSample;
                        ...
                        try
                        {
                            ...
                            var samplesInBuffer = streaminfo.MaxBlockSize;
                            pcmBuffer = new byte[bytesPerInterChannelSample * samplesInBuffer];
                            long currentSample = 0;
                            var samples = new int[streaminfo.ChannelsCount * samplesInBuffer];
                            ...
                            {
                                var samplesLeft = (int)(totalSamples - currentSample);
                    ...
                    {
                        var dataBlockSize = BitConverter.ToUInt32(blockHeader, 4);
                        var extraData = new byte[(int)dataBlockSize];
