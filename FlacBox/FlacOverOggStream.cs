...
namespace FlacBox
{
    ...
    public class FlacOverOggStream : Stream, IPageble
    {
        ...
        public FlacOverOggStream(Stream stream, FlacOverOggStreamMode mode)
        {
            ...
            switch (mode)
            {
                case FlacOverOggStreamMode.Decode:
                    dataSource = ReadFlacStream(stream);
        ...
        private void InitializeWriteMode()
        {
            ...
            streamId = BitConverter.ToUInt32(randomBytes, 0);
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            {
                var tail = MaxFrameDataLength - currentData.Count;
            ...
            Debug.Assert(count <= MaxFrameDataLength);
...
#pragma warning disable 0414 // TODO remove
        ...
        private void WriteFullFrame(byte[] buffer, int offset, int count, bool lastFrame, bool overflow)
        {
            ...
            var segments = (count + 254) / 255;
            var frameHeader = new byte[27 + segments];
            ...
            var crc32 = CrcUtils.Crc32(0, frameHeader);
        ...
        private IEnumerator<ArraySegment<byte>> ReadFlacStream(Stream stream)
        {
            ...
            var pageHeader = new byte[PageHeaderSize];
            var read = stream.Read(pageHeader, 0, PageHeaderSize);
            ...
            while (read > 0)
            {
                ...
                var absolutePosition = BitConverter.ToUInt64(pageHeader, 6);
                var currentStreamId = BitConverter.ToUInt32(pageHeader, 14);
                var pageNo = BitConverter.ToUInt32(pageHeader, 18);
                var pageCrc = BitConverter.ToUInt32(pageHeader, 22);
                int segmentsCount = pageHeader[26];

                var segmentSizes = new byte[segmentsCount];
                ...
                else
                {
                    ...
                    var crc = Crc32(Crc32(0, pageHeader), segmentSizes);
                    ...
                    {
                        // check if it's FLAC
                        var isFlac = data.Length == FlacFirstPageLength &&
                            data[0] == 0x7F && data[1] == 'F' && data[2] == 'L' && data[3] == 'A' && data[4] == 'C';
                        var isDummyFlac = data.Length >= 4 && data[0] == 'f' && data[1] == 'L' && data[2] == 'a' && data[3] == 'C';
                        if (isFlac)
                        {
                            ...
                            var numberOfNonAudioPackets = data[7] << 8 | data[8];
